	...
	for (size_t i = 0; i < inputExpresion.length(); i++)
	{
		...
		if (isOperator(charTostring))
		{
			...
			else if (inputExpresion[i] == ')' || inputExpresion[i] == '(')
			{
				...
				else  if (inputExpresion[i] == ')' && openFrame >= 0) // ??????????? ??? ??????? ?? ????? ? output 
				{
					openFrame--;
...
void PrinTree(TreeLeaf* currNode, string deep)
{
	if (currNode != NULL)//???? ?? ?????????? ?????? ????
	{
		cout << deep + currNode->value << endl; //?????????? ?????? ??????
		deep += "_|";
		PrinTree(currNode->right, deep); //??????????? ??????? ??? ??????? ?????????
		PrinTree(currNode->left, deep); //??????????? ??????? ??? ?????? ?????????
	}
}
